...
if [[ "$DESKTOP" == "yes" ]]; then
    ...
    echo ">>> First boot lands at TTY for armbian-firstrun (set root password, create user)."
    echo ">>> Run 03-setup.sh on the booted system to enable auto-start UI / migrate to NVMe."
...
# kubuntu-desktop's postinst flips the default target to graphical, which
# makes SDDM grab tty1 before armbian-firstrun can prompt for root password
# and user creation. Force back to multi-user so first boot lands at a text
# console; 03-setup.sh asks the user later if they want to enable auto-start
# graphical (sets default back to graphical.target).
